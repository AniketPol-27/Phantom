...
def _safe_numeric(value):
    """
    Normalize FHIR-derived values into float.
    ...
    if isinstance(value, dict):
        ...
        if "value" in value:
            return _safe_numeric(value["value"])

        if (
            "valueQuantity" in value
            and isinstance(value["valueQuantity"], dict)
        ):
            return _safe_numeric(
                value["valueQuantity"].get("value")
            )
...
def build_renal_system(
    ...
    latest_egfr_obs = get_latest_lab_value(
        lab_observations,
        "egfr",
    )

    latest_creatinine_obs = get_latest_lab_value(
        lab_observations,
        "creatinine",
    )
    ...
    if latest_creatinine_obs:
        ...
        creatinine_value = _safe_numeric(
            latest_creatinine_obs["value"]
        )
    ...
    if latest_egfr_obs:
        ...
        current_egfr = _safe_numeric(
            latest_egfr_obs["value"]
        )
        ...
        try:
            ...
            current_egfr = calculate_egfr_ckd_epi_2021(
                serum_creatinine=_safe_numeric(
                    latest_creatinine_obs["value"]
                ),
                age=age,
                sex=sex,
    ...
    if current_egfr is not None:
        ...
        try:
            ...
            ckd_stage_info = classify_ckd_stage(
                current_egfr
            )
            ...
            logger.warning(
                "renal.ckd_stage_classification_failed",
                error=str(e),
            )
    ...
    egfr_history = get_lab_value_history(
        lab_observations,
        "egfr",
    )
    ...
    if not egfr_history and age and sex:
        ...
        creatinine_history = get_lab_value_history(
            lab_observations,
            "creatinine",
        )
    ...
    latest_uacr = get_latest_lab_value(
        lab_observations,
        "uacr",
    )
    ...
    if latest_uacr:
        ...
        try:
            ...
            albuminuria_info = classify_albuminuria(
                _safe_numeric(
                    latest_uacr["value"]
                )
            )
            ...
            logger.warning(
                "renal.albuminuria_classification_failed",
                error=str(e),
            )
    ...
    if ckd_stage_info and albuminuria_info:
        ...
        try:
            ...
            kdigo_risk = kdigo_risk_matrix(
                ...
                albuminuria_category=albuminuria_info.get(
                    "category"
                ),
            )
    ...
    active_classes = get_active_drug_classes(
        active_medications
    )

    has_acei_arb = (
        "ace_inhibitor" in active_classes
        or "arb" in active_classes
    )

    has_sglt2i = (
        "sglt2_inhibitor" in active_classes
    )
    ...
    renoprotective_coverage = {
        ...
        "coverage_score":
            _renoprotective_coverage_score(
                has_acei_arb=has_acei_arb,
                has_sglt2i=has_sglt2i,
                has_diabetes=has_condition(
                    active_conditions,
                    "type_2_diabetes",
                ),
                has_ckd=has_condition(
                    active_conditions,
                    "chronic_kidney_disease",
                ),
                albuminuria_category=(
                    albuminuria_info.get("category")
                    if albuminuria_info
                    else None
                ),
            ),
    ...
    if (
        ...
        and trajectory
        and trajectory.get("slope_per_year")
        is not None
    ):
    ...
    return {
        ...
        "creatinine": {
            ...
            "unit": (
                latest_creatinine_obs.get("unit")
                if latest_creatinine_obs
                else None
            ),
...
def _renoprotective_coverage_score(
    ...
    """
    Classify renoprotective coverage.
    """
    ...
    if (
        albuminuria_category in ("A2", "A3")
        and not has_acei_arb
    ):
